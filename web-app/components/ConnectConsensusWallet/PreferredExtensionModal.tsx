...
const WalletOption: FC<WalletOptionProps> = ({ wallet, index, onClose }) => {
  ...
  return (
    <button
      ...
      className='cursor-pointer flex flex-col gap-3 rounded-md border border-brand-secondary p-2 hover:bg-gray-100 dark:hover:bg-transparent/10 dark:border-gray-100'>
      <div className='flex items-center gap-3'>
        <Image src={icon} alt={wallet.title} width={32} height={32} />
        <div className='text-center text-xs font-normal text-gray-900 dark:text-white'>{wallet?.title}</div>
      </div>
      {wallet?.installed ? (
        <></>
      ) : (
        <div className='justify-self-end text-xs font-medium text-brand-secondary dark:text-gray-100'>
  ...
  const supportedWallets = useMemo(
    () =>
      uniqueWallets.filter(
        (item) => (Object.values(SupportedWalletExtension) as string[]).includes(item.extensionName) && item
      ),
    [uniqueWallets]
  )
  ...
  return (
    <Modal title='Select your extension' onClose={onClose} isOpen={isOpen}>
      <div className='flex flex-col items-start gap-4'>
        ...
        <button
          className='cursor-pointer flex w-full max-w-fit items-center gap-2 rounded-full bg-gray-darker px-2 text-sm font-medium text-white dark:border dark:border-gray-100 dark:bg-transparent md:space-x-4 md:text-base'
          onClick={onClose}>
          Close
        </button>
      </div>
